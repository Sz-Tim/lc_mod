...
library(sevcheck)
p_load("tidyverse", "magrittr", "forcats", "rstan", 
       "ggmcmc", "stringr", "fields")
...
maxGridCol <- 7  # number of blocks per column; 1-54
...
d <- list(n1=nFit, n2=nFit+1, n3=n$tot, L=6, nB_d=nBd, nB_p=nBp,
          ...
          M=(1+2*M_r)^2-1, nn_id=t(nn_id), nn_d=nn_d, 
          nn_dM=t(nn_dM), nn_dim=nn_dim,
          X_d1=Xd[[1]], X_d2=Xd[[2]], X_d3=Xd[[3]], X_d4=Xd[[4]], X_p=Xp)
stan_rdump(ls(d), file="code/all_data.Rdump", envir=list2env(d))
out <- stan(file="code/latent_qrBeta_NNGP.stan", init=0, thin=5,
            data=read_rdump("code/all_data.Rdump"), 
            iter=500, warmup=200, chains=3, seed=4337, refresh=20,
