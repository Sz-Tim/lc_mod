...
Xd[[2]] <- cbind(scale(rdLen$rdLen[n$all]), 
                 scale(pop00$popTot[n$all]),
                 scale(clim$b7[n$all]))
Xd[[3]] <- cbind(scale(pop00$popTot[n$all]),
                 scale(clim$b7[n$all]))
Xd[[4]] <- cbind(scale(clim$b1[n$all]), 
                 scale(clim$b12[n$all]),
                 scale(hous00$secHome[n$all]))
...
out <- stan(file="code/LC_mod_qr_d.stan", data=d, init=0, thin=50, 
            iter=10000, warmup=6000, chains=4, seed=433337, refresh=500,
            include=FALSE, pars=c("nu_Y1", "nu_Y2", "pX"),
