...
transformed parameters {
  ...
  vector[L-2] nu_ds[n3];
  vector<lower=0, upper=1>[n3] p;  // pWP
  vector[n3] nu_p;
  //betas
  vector[nB_p] beta_p;
  ...
  nu_p = to_vector(nu[,4]) ./ (to_vector(nu[,4]) + to_vector(nu[,5]));
  
  //estimate bias & lump WP [,4] and Evg [,5]
  ////fit betas using cells with Y1 & Y2
  p[1:n1] = inv_logit(nu_p[1:n1] + Q_p[1:n1,] * theta_p);
  ...
  ////estimate bias in cells with only Y2 using fit betas
  {
    ...
    b_d = theta_d;
    b_p = theta_p;
    p[n2:n3] = inv_logit(nu_p[n2:n3] + Q_p[n2:n3,] * b_p);
