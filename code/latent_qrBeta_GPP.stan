...
transformed parameters {
  ...
  for(l in 1:(L-1)) {
    ...
    matrix[n3,m] C_site_inv_Cstar;
    matrix[m,m] D_star_phi_etasq;
  
    eta_sq[l] = pow(eta[l], 2);
    sig_sq[l] = pow(sigma[l], 2);
    Cstar = diag_matrix(rep_vector(eta_sq[l] + sig_sq[l], m));
    D_star_phi_etasq = eta_sq[l] * exp(-D_star * phi[l]);
    for(i in 1:(m-1)) {
	    for(j in (i+1):m) {
		    Cstar[i,j] = D_star_phi_etasq[i,j];
		    Cstar[j,i] = Cstar[i,j];
	    }
    }
    w_star = cholesky_decompose(Cstar) * w_z[l];
  
    //latent gp at sample locations + bias adjustment from Finley et al. 2009
    C_site_star = eta_sq[l] * exp(-D_site_star * phi[l]);
    // efficient and arithmetically stable for: C_site_star * inverse(Cstar)
    C_site_inv_Cstar = mdivide_right_spd(C_site_star, Cstar);
    w[l] = C_site_inv_Cstar * w_star
          + (e_z[l] 
            .* sqrt(eta_sq[l] + sig_sq[l]
                    - rows_dot_product(C_site_inv_Cstar, C_site_star)));
  }  
