...
```{r veg_SB1_6}
time.df <- tibble(n=c(25, 50, 100, 500, 1000, 2000, 5000),
                  m1=NA, m2=NA)
time.ls <- vector("list", length=nrow(time.df))
...
d_SB1 <- list(N=nrow(nu), L=ncol(nu), Y1=Y1, Y2=Y2, nB_d=nB.d, nB_p=nB.p,
              ...
              X_d4=X.d[[4]], X_d5=X.d[[5]], X_p=X.p, mu=0, tau=2.5)
...
time.df$m1[r] <- mod.time.2[3]
time.df$m2[r] <- mod.time.3[3]
time.ls[[r]] <- get_elapsed_time(out2)
# time.df$m3[r] <- mod.time.3[3]
cat("Finished row", r, "with", nCell, "cells: ", mod.time.2[3], "\n")
}

map(time.ls, as_tibble) %>% map_df(`[`, .id="n") %>% 
  mutate(tot=(warmup + sample), perCell=tot/as.numeric(n)) %>%
  ggplot(aes(x=as.numeric(n), y=perCell)) + geom_point()
