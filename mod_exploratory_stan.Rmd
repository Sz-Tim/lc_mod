...
# First attempt with Y2-only cells
```{r pred_test}
# data from test grid (1-acre composition)
...
clim <- read_csv(paste0("data/clim_", grdSz, ".csv")) %>% 
  ...
  summarise(bio1=mean(bio1_mean), bio7=mean(bio7_mean), bio12=mean(bio12_mean)) %>% 
  ungroup %>% mutate(tmean=scale(bio1), tseas=scale(bio7), precip=scale(bio12))
...
dev.df <- data.frame(Yd1=Ydiff[,1], Yd2=Ydiff[,2], Yd3=Ydiff[,3], 
                     Yd4=Ydiff[,4], Yd5=Ydiff[,5],
                     pWP=grnt[,4]/(grnt[,4]+grnt[,5]+0.001),
                     rLen=rdLen$lenSc, pop00=pop00$popSc,
                     secHome=hous00$secHomeSc,
                     tmean=clim$tmean, tseas=clim$tseas, precip=clim$precip)
...
## initialize
nBd <- c(3, 2, 1, 3) 
BCov <- c(1, 1, 1, 1)  # which deltas to model
Xd <- vector("list", nLC-2)
## fill
Xd[[1]] <- cbind(rdLen$lenSc[allCell], pop00$popSc[allCell], clim$tmean[allCell])
Xd[[2]] <- cbind(rdLen$lenSc[allCell], pop00$popSc[allCell])
Xd[[3]] <- matrix(clim$tseas[allCell], ncol=1)
Xd[[4]] <- cbind(clim$tmean[allCell], clim$precip[allCell], rdLen$lenSc[allCell])

# covariates: WP|Evg
## initialize
nBp <- 2
Xp <- cbind(clim$tmean[allCell], clim$precip[allCell])
pWp <- dev.df$pWP[allCell]
...
gg.EvgMed <- gg.EvgComb %>% group_by(Cell, LC, Set, Parameter) %>%
  summarise(med=median(value), q05=quantile(value, 0.05), q25=quantile(value, 0.25),
            q75=quantile(value, 0.75), q95=quantile(value, 0.95),
            Y1=first(Y1), Y2=first(Y2)) 
...
ggplot(gg.EvgMed, aes(x=Y1, xend=Y1, y=Y2, yend=med, colour=abs(Y2-Y1)<abs(med-Y1))) +
    xlim(0,1) + ylim(0,1) +
    geom_abline(slope=1, linetype=3) + scale_colour_manual(values=c("darkgreen", "red")) +
    geom_segment(arrow=arrow(length=unit(0.1, "cm")), alpha=0.4) + facet_grid(Set~LC) +
    labs(x="Y1", y="Y2 -> median") + theme(legend.position="none")
