...
tr_gjam <- function(eta, a=0.95) {
  ...
  w[w==0] <- rtnorm(sum(w==0), mean=0, sd=0.25, upper=0)
  w
}
...
out3 <- stan(file="mod_sandbox/veg_SB1_2-3.stan", data=d_SB1.2, thin=50)#, init=inits)
...
gg.nu <- gg.nu %>% rename(value.1=value) %>%
  ...
  mutate(value.1.eta=tr_gjam_inv(value.1), value.1.scaled=value.1/sum(value.1),
         value.1.ZY=tr_lein(value.1), value.2.eta=tr_gjam_inv(value.2),
...
gg.nu <- gg.nu %>% group_by(Iteration, Chain, Cell) %>%
  mutate(value.eta=tr_gjam_inv(value), value.scaled=value/sum(value),
...
gg.med <- gg.nu %>% group_by(Cell, LC, Parameter) %>%
  ...
  mutate(med.eta=tr_gjam_inv(med), med.scaled=med/sum(med),
         med.ZY=tr_lein(med))
