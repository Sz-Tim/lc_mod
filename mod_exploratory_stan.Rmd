...
```{r veg_SB1_6}
time.df <- tibble(n=c(25, 100, 500, 1000, 2000))#, 5000, 10000))
time.ls <- vector("list", length=4) 
names(time.ls) <- c("repar_inline", "cauch_inline",
                    "repar_sep", "cauch_sep")
...
## covariance matrices
cov.grnt <- cov(GRNT)/15
cov.nlcd <- cov(GRNT)/15
...
out <- stan(file="mod_sandbox/veg_SB1_6-3_repar_inline.stan", 
            data=d_SB1, thin=50, init=0, chains=8, seed=874)#,
            # control=list(adapt_delta=0.95, max_treedepth=15))
})
mod.time.2 <- system.time({
out2 <- stan(file="mod_sandbox/veg_SB1_6-3_cauch_inline.stan",
             data=d_SB1, thin=50, init=0, chains=8, seed=874)#,
            # control=list(adapt_delta=0.95, max_treedepth=15))
})
