...
```{r veg_SB1_6}
time.df <- tibble(n=c(25))#, 1000, 1000, 2000, 5000, 10000))
time.ls <- vector("list", length=2) 
names(time.ls) <- c("cauch0_Lmx0", "cauch_Lmx0_no6") #"cauch0_Lmx1",
                    # "cauch1_Lmx0")
...
# run model
d_SB1_no6 <- list(N=nrow(nu), L=ncol(nu), Y1=Y1[,-6], Y2=Y2[,-5], 
              nB_d=nB.d, nB_p=nB.p,
...
out <- stan(file="mod_sandbox/6-3_cauch0_Lmx0_no6.stan",
            data=d_SB1_no6, thin=50, init=0, chains=2, seed=334561)#,
            # control=list(adapt_delta=0.95, max_treedepth=15))
})
mod.time.2 <- system.time({
out2 <- stan(file="mod_sandbox/6-3_cauch0_Lmx0.stan",
             data=d_SB1, thin=50, init=0, chains=2, seed=334561)#,
            # control=list(adapt_delta=0.95, max_treedepth=15))
})
mod.time.3 <- system.time({
out3 <- stan(file="mod_sandbox/6-3_cauch1_Lmx0.stan",
             data=d_SB1, thin=50, init=0, chains=2, seed=334561)#,
            # control=list(adapt_delta=0.95, max_treedepth=15))
})
# mod.time.4 <- system.time({
# out4 <- stan(file="mod_sandbox/veg_SB1_6-3_Lmx_noEvg.stan",
#              data=d_SB1, thin=50, init=0, chains=4, seed=31)#,
#             # control=list(adapt_delta=0.95, max_treedepth=15))
# })
...
time.ls %>% at_depth(2, as_tibble) %>% 
  ...
  geom_point() + scale_colour_brewer(type="qual", palette=2) + 
...
```{r}
cells <- sample(1:nrow(GRNT), nCell)
p.nu <- GRNT[cells,4]/(GRNT[cells,4]+GRNT[cells,5]+0.0001)
p.nu[p.nu==0] <- 0.0001; p.nu[p.nu==1] <- 0.9999
X.p <- matrix(nrow=nCell, ncol=nB.p)
X.p[,1] <- logit(p.nu)/B.p[1] - B.p[2] + rnorm(nCell, 0, 0.5)
X.p[,2] <- 1

d_SB1 <- list(N=nCell, L=6, 
              Y1=GRNT[cells,-6], Y2=NLCD[cells,-5], 
              nB_d=nB.d, nB_p=nB.p,
              X_d1=X.d[[1]], X_d2=X.d[[2]], X_d3=X.d[[3]], 
              X_d4=X.d[[4]], X_d5=X.d[[5]], X_p=X.p)
out <- stan(file="mod_sandbox/6-3_cauch0_Lmx0.stan", iter=2000,
            data=d_SB1, thin=50, init=0, chains=4, seed=33456)
gg.nu <- ggs(out, "n_eta") %>% arrange(Parameter, Chain, Iteration)
nGG <- attr(gg.nu, "nChains")*attr(gg.nu, "nIterations")
gg.nu %<>% rename(value.1=value) %>%
  mutate(Y1 = t(GRNT[cells,]) %>% c %>% rep(each=nGG),
         Y2 = t(cbind(d_SB1$Y2[,1:3], d_SB1$Y2[,4]*antilogit(X.p %*% B.p),
                      d_SB1$Y2[,4]*(1-antilogit(X.p %*% B.p)), 
                      NLCD[cells,5])) %>%
           c %>% rep(each=nGG),
         LC = 1:6 %>% rep(each=nGG) %>% rep(times=nCell),
         Cell = 1:nCell %>% rep(each=nGG*6))
gg.med <- gg.nu %>% group_by(Cell, LC, Parameter) %>%
  summarise(med.1=median(value.1), Y1=first(Y1), Y2=first(Y2)) %>%
  ungroup() %>% group_by(Cell)
ggplot(gg.med, aes(x=Y1, y=med.1)) +  geom_abline(slope=1, linetype=2) +
  geom_point(alpha=0.5) + facet_wrap(~LC) + ylim(0,1)
ggplot(gg.med, aes(x=Y2, y=med.1)) +  geom_abline(slope=1, linetype=2) +
  geom_point(alpha=0.5) + facet_wrap(~LC) + ylim(0,1)
ggplot(gg.med, aes(x=Y1, y=Y2)) +  geom_abline(slope=1, linetype=2) +
  geom_point(alpha=0.5) + facet_wrap(~LC) + ylim(0,1)
ggplot(gg.med, aes(x=(Y1+Y2)/2, y=med.1)) +  geom_abline(slope=1, linetype=2) +
  geom_point(alpha=0.5) + facet_wrap(~LC) + ylim(0,1)

ggplot(gg.nu, aes(x=value.1)) + geom_histogram() + facet_wrap(~LC)
ggplot(gg.nu, aes(x=Y1)) + geom_histogram() + facet_wrap(~LC)
ggplot(gg.nu, aes(x=(Y1+Y2)/2)) + geom_histogram() + facet_wrap(~LC)

ggplot(gg.nu, aes(x=Y1, y=value.1, colour=factor(Chain))) +  geom_abline(slope=1, linetype=2) +
  geom_point(alpha=0.05) + facet_wrap(~LC) + ylim(0,1)
ggplot(gg.nu, aes(x=Y2, y=value.1)) +  geom_abline(slope=1, linetype=2) +
  geom_point(alpha=0.05) + facet_wrap(~LC) + ylim(0,1)
ggplot(gg.nu, aes(x=(Y1+Y2)/2, y=value.1)) +  geom_abline(slope=1, linetype=2) +
  geom_point(alpha=0.05) + facet_wrap(~LC) + ylim(0,1)
...
gg.nu %<>% rename(value.1=value) %>%
  ...
  ungroup() %>% group_by(Cell)
gg.new <- ggs(out, "Y2_ds_new") %>% arrange(Parameter, Chain, Iteration)
...
gg.new %<>% rename(value.1=value) %>%
  mutate(nuNew = t(nuNew) %>% c %>% rep(each=nGG),
         Y2New = t(cbind(Y2New[,1:3], Y2New[,4]*antilogit(X.pNew %*% B.p),
                      ...
                      # Y2New[,5])) %>%
           c %>% rep(each=nGG),
         LC = 1:5 %>% rep(each=nGG) %>% rep(times=nNew),
         Cell = 1:nNew %>% rep(each=nGG*5)) %>%
  group_by(Cell, LC, Parameter) %>%
