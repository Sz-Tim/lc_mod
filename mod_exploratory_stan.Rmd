...
```{r veg_SB1_6}
time.df <- tibble(n=c(10, 50, 100))#, 1000, 2000, 5000, 10000))
time.ls <- vector("list", length=2) 
names(time.ls) <- c("repar_inline", "repar_inline_constr")#,
                    # "repar_sep", "cauch_sep")
...
out <- stan(file="mod_sandbox/veg_SB1_6-3_repar_inline.stan", 
            data=d_SB1, thin=50, init=0, chains=4, seed=874)#,
            # control=list(adapt_delta=0.95, max_treedepth=15))
})
mod.time.2 <- system.time({
out2 <- stan(file="mod_sandbox/veg_SB1_6-3_repar_inline_constr.stan", 
             data=d_SB1, thin=50, init=0, chains=4, seed=874)#, 
            # control=list(adapt_delta=0.95, max_treedepth=15))
})
# mod.time.3 <- system.time({
# out3 <- stan(file="mod_sandbox/veg_SB1_6-3_repar_sep.stan", 
#              data=d_SB1, thin=50, init=0, chains=2, seed=874)#, 
#             # control=list(adapt_delta=0.95, max_treedepth=15))
# })
# mod.time.4 <- system.time({
# out4 <- stan(file="mod_sandbox/veg_SB1_6-3_cauch_sep.stan", 
#              data=d_SB1, thin=50, init=0, chains=2, seed=874)#, 
#             # control=list(adapt_delta=0.95, max_treedepth=15))
# })
...
time.ls %>% at_depth(2, as_tibble) %>% 
  ...
  ggplot(aes(x=as.numeric(n), y=perCell, colour=model, group=model)) + 
...
gg.nu <- gg.nu %>% rename(value.1=value) %>%
  mutate(nu.true = t(nu) %>% c %>% rep(each=nGG),
         ...
         value.2 = ggs(out2, "n_eta") %>% 
           arrange(Parameter, Chain, Iteration) %>% .$value,
         value.3 = ggs(out3, "n_eta") %>% 
           arrange(Parameter, Chain, Iteration) %>% .$value,
         value.4 = ggs(out4, "n_eta") %>% 
           arrange(Parameter, Chain, Iteration) %>% .$value)
ggplot(gg.nu, aes(x=nu.true, y=value.1)) + geom_abline(slope=1, linetype=2) +
