...
for(i in 1:nCell) {
  ...
  Y2.dp[i,] <- rmvnorm(1, nu[i,], cov.grnt) %>% tr_gjam()
}
plot(Y1, Y2.dp); plot(nu, Y1); plot(nu, Y2.dp)
d_SB1.2 <- list(N=nrow(nu), L=ncol(nu), Y1=Y1, Y2=Y2.dp)
out <- stan(file="mod_sandbox/veg_SB1_2-1.stan", data=d_SB1.2, thin=50)
