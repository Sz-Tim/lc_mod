...
```{r realData_test}
# data from test grid
grnt <- read.csv("data/out_01_1a_grnt.csv") %>% as.matrix
nlcd <- read.csv("data/out_01_1a_nlcd.csv") %>% as.matrix
road.df <- read_csv("data/roads_01_1a.csv") %>%
  mutate(lenSc=scale(roadLen), ctSc=scale(roadCt))
...
## fill
Xd[[1]] <- cbind(road.df$lenSc[allCell], road.df$ctSc[allCell])
for(i in 2:(nLC-1)) {
...
out <- stan(file="mod_sandbox/6-3_newCells_InMod.stan", iter=5000, 
            data=d_SB1, thin=50, init=0, chains=4, seed=4337)
gg.nu <- ggs(out, "n_eta") %>% 
  # filter(!grepl("_new", .$Parameter)) %>%
  arrange(Parameter, Chain, Iteration)
...
ggplot(gg.nu) + xlim(-1,1) + facet_wrap(~LC, scales="free_y") + 
  geom_vline(xintercept=0, linetype=3) +
  geom_density(aes(x=(med-Y1)-(Y2-Y1), colour=Set))

ggplot(gg.nu, aes(x=Y1.fit, y=med)) + geom_point(alpha=0.2) + 
  geom_abline(slope=1, linetype=3) + facet_wrap(~LC) + xlim(0,1) + ylim(0,1)
ggplot(gg.nu, aes(x=Y2.fit, y=med)) + geom_point(alpha=0.2) + 
  geom_abline(slope=1, linetype=3) + facet_wrap(~LC) + xlim(0,1) + ylim(0,1)
ggplot(gg.nu, aes(x=Y1.fit, y=Y2.fit)) + geom_point(alpha=0.2) + 
  geom_abline(slope=1, linetype=3) + facet_wrap(~LC) + xlim(0,1) + ylim(0,1)

ggplot(gg.new, aes(x=Y1.new, y=med)) + geom_point(alpha=0.2) + 
  geom_abline(slope=1, linetype=3) + facet_wrap(~LC) + xlim(0,1) + ylim(0,1)
ggplot(gg.new, aes(x=Y2.new, y=med)) + geom_point(alpha=0.2) + 
  geom_abline(slope=1, linetype=3) + facet_wrap(~LC) + xlim(0,1) + ylim(0,1)
ggplot(gg.new, aes(x=Y1.new, y=Y2.new)) + geom_point(alpha=0.2) + 
  geom_abline(slope=1, linetype=3) + facet_wrap(~LC) + xlim(0,1) + ylim(0,1)
```