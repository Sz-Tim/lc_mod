...
library(sevcheck); library(pacman)
...
mod <- mlp(X.train, Y.train, size=9, maxit=75000)

# combine and predict
fit.df <- tibble(Y1=c(as.matrix(Y.train)),
                 pred.m=c(as.matrix(mod$fitted.values)),
                 LC=rep(1:6, each=nrow(Y.train)),
                 Y2=c(as.matrix(cbind(X.train[,1:3], 
                                      X.train[,4]*X.train[,6],
                                      X.train[,4]*(1-X.train[,6]),
                                      X.train[,5]))),
                 BlockRow=rep(X$BlockRow[index], 6),
                 BlockCol=rep(X$BlockCol[index], 6),
                 Set="Y1+Y2") %>% 
  bind_rows(tibble(Y1=c(as.matrix(Y.test)),
                pred.m=c(predict(mod, X.test)),
                LC=rep(1:6, each=nrow(Y.test)),
...
# plots
ggplot(fit.df, aes(x=Y1, y=Y2)) + geom_point(alpha=0.5) +
  xlim(0,1) + ylim(0,1) +
