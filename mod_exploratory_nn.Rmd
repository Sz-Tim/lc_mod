...
mod2 <- mlp(rand.train, resp.train, size=9, maxit=10000)
fit.df <- data.frame(Y1=c(as.matrix(resp.train)),
                     pred.m2=c(as.matrix(mod2$fitted.values)),
                     LC=rep(1:6, each=nrow(resp.train)),
                     Y2=c(as.matrix(cbind(rand.train[,1:3], 
                               rand.train[,4]*rand.train[,6],
                               rand.train[,4]*(1-rand.train[,6]),
                               rand.train[,5]))))
pr.df <- data.frame(Y1=c(as.matrix(resp.test)),
                    ...
                    Y2=c(as.matrix(cbind(rand.test[,1:3], 
                               ...
                               rand.test[,5]))))
ggplot(fit.df, aes(x=Y1, y=pred.m2)) + geom_point(alpha=0.5) +
  xlim(0,1) + ylim(0,1) +
